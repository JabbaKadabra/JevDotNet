...
using JevDotNet;
using JevDotNet.Verification;

var runner = new CheckRunner();
Console.WriteLine("JevDotNet verification runner");
Console.WriteLine();
...
await runner.CheckAsync("direct choice request sends auth, model, and question", async () =>
{
    ...
    using var jev = new Jev("secret-key", null, new HttpClient(handler));
...
{
    var handler = FakeHandler.Json(DirectPayload);
    using var jev = new Jev("secret-key", new JevOptions
    {
...
await runner.CheckAsync("returned options keep their reference identity", async () =>
{
    ...
    var answer = await jev.ChoiceAsync("ticket", "Which team?", teams);
...
await runner.CheckAsync("a choice option with 10 properties is accepted and 11 is rejected", async () =>
{
    ...
    using var jev = new Jev("secret-key", new JevOptions { MaxChoiceProperties = 10 }, new HttpClient(handler));
    ...
    await Expect.ThrowsAsync<JevValidationException>(
        () => jev.ChoiceAsync("ticket", "Which option?", new[] { new ElevenProperties() }),
...
await runner.CheckAsync("the property limit error is actionable", async () =>
{
    ...
    try
    {
        await jev.ChoiceAsync<object>("ticket", "Which option?", new object[] { new Point(), new TwentyOneProperties() });
...
await runner.CheckAsync("nested objects and arrays are counted and ignored properties are not", async () =>
{
    ...
    await Expect.ThrowsAsync<JevValidationException>(
        () => jev.ChoiceAsync("ticket", "Which option?", new[] { new NestedTen() }),
        "Nested properties should be counted.");
    await Expect.ThrowsAsync<JevValidationException>(
        () => jev.ChoiceAsync("ticket", "Which option?", new[] { new WithArray { Points = Enumerable.Repeat(new Point(), 11).ToArray() } }),
        "Array elements should be counted.");
    await Expect.ThrowsAsync<JevValidationException>(
        () => jev.ChoiceAsync("ticket", "Which option?", new[] { new WithDictionary { Points = Enumerable.Range(0, 11).ToDictionary(index => "p" + index, _ => new Point()) } }),
        "Dictionary entries should be counted.");
    await Expect.ThrowsAsync<JevValidationException>(
        () => jev.ChoiceAsync("ticket", "Which option?", new[] { Cyclic() }),
...
await runner.CheckAsync("cancellation before the request is propagated and sends nothing", async () =>
{
    ...
    await Expect.ThrowsAsync<OperationCanceledException>(
        () => jev.NoulAsync("ticket", "Is it urgent?", cancellation.Token),
...
await runner.CheckAsync("cancellation during HTTP work is propagated unchanged", async () =>
{
    ...
    try
    {
        await jev.NoulAsync("ticket", "Is it urgent?", cancellation.Token);
...
await runner.CheckAsync("unsuccessful responses throw with status and body", async () =>
{
    ...
    using var jev = new Jev("bad-key", null, new HttpClient(handler));
...
await runner.CheckAsync("malformed and incomplete responses are rejected", async () =>
{
    ...
    using (var jev = new Jev("secret-key", null, new HttpClient(malformed)))
    {
        await Expect.ThrowsAsync<JevProtocolException>(
            () => jev.NoulAsync("ticket", "Is it urgent?"),
    ...
    using (var jev = new Jev("secret-key", null, new HttpClient(wrongType)))
    {
        await Expect.ThrowsAsync<JevProtocolException>(
            () => jev.NoulAsync("ticket", "Is it urgent?"),
    ...
    using (var jev = new Jev("secret-key", null, new HttpClient(unknownChoice)))
    {
        await Expect.ThrowsAsync<JevProtocolException>(
            () => jev.ChoiceAsync("ticket", "Which team?", new[] { "billing" }),
...
await runner.CheckAsync("a supplied HTTP client is used and stays alive after disposal", async () =>
{
    ...
    var jev = new Jev("secret-key", null, httpClient);
...
await runner.CheckAsync("local validation failures send no request", async () =>
{
    ...
    await Expect.ThrowsAsync<JevValidationException>(
        () => jev.Query("ticket").SendAsync(),
        "An empty batch should be rejected.");
    await Expect.ThrowsAsync<JevValidationException>(
        () => jev.Query("ticket").Noul(new Noul("same", "First?")).Noul(new Noul("same", "Second?")).SendAsync(),
...
{
    var handler = FakeHandler.Json(DirectPayload);
    var jev = new Jev("secret-key", null, new HttpClient(handler));
    jev.Dispose();
