...
namespace JevDotNet
{
    ...
    public sealed class Jev : IDisposable
    {
        ...
        /// <summary>Initializes a new instance of the <see cref="Jev"/> class.</summary>
        ...
        {
            _apiKey = Guard.ApiKey(apiKey);
            _options = options ?? new JevOptions();
            JevSettings.From(_options);

            _ownsHttpClient = httpClient == null;
            _httpClient = httpClient ?? new HttpClient();
        }

        /// <summary>Evaluates a single question against a state.</summary>
        ...
        /// <summary>Starts a batch of questions to evaluate against a state.</summary>
        /// <param name="state">The content to evaluate: a string, a POCO, or an array.</param>
        ...
        public JevQuery Query(object state)
        {
            ...
            if (state == null)
            {
                throw new ArgumentNullException(nameof(state), "The state to evaluate must not be null.");
            }

            return new JevQuery(this, state);
        }

        /// <summary>Evaluates a choice question against a state.</summary>
        ...
        /// <summary>Evaluates a score question against a state.</summary>
        /// <param name="state">The content to evaluate: a string, a POCO, or an array.</param>
        /// <param name="instructions">The question the model answers.</param>
        ...
        /// <summary>Evaluates a noul (yes/no) question against a state.</summary>
        /// <param name="state">The content to evaluate: a string, a POCO, or an array.</param>
        ...
        /// <summary>Releases the internal HTTP client when this instance owns it.</summary>
        public void Dispose()
        {
            if (_disposed)
            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var settings = JevSettings.From(_options);
