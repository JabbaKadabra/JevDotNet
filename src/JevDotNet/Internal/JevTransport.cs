...
namespace JevDotNet
{
    ...
    internal static class JevTransport
    {
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Post, endpoint))
            {
                    ...
                    .ConfigureAwait(false))
                {
                    if (!response.IsSuccessStatusCode)
                    {
                        ...
                        throw new JevApiException(response.StatusCode, Truncate(errorBody));
                    }
        ...
        private static string Truncate(string body)
        {
            ...
            return body.Substring(0, MaxErrorBodyLength) +
                   string.Format(CultureInfo.InvariantCulture, "... ({0} characters truncated)", body.Length - MaxErrorBodyLength);
        }
    }
}
