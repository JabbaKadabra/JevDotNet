...
A runnable console app that shows how to use the `JevDotNet` client against the live
...
dotnet run --project samples/JevDotNet.Sample
```
...
| Variable | Purpose |
| --- | --- |
| `JEV_API_KEY` | Required. Sent as a bearer token on every request. |
| `JEV_MODEL` | Optional. Model that handles requests. Defaults to `jev-latest`. |

Pass a scenario name to run only that part:

```bash
dotnet run --project samples/JevDotNet.Sample -- quickstart
dotnet run --project samples/JevDotNet.Sample -- --help
```
