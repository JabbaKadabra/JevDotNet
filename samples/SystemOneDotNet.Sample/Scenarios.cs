...
public static class Scenarios
{
    ...
    private static async Task QuickStartAsync(ISystemOneClient systemOne, CancellationToken cancellationToken)
    {
        ...
        NoulAnswer urgency = await systemOne.NoulAsync(
            ticket,
    ...
    private static async Task StructuredOptionsAsync(ISystemOneClient systemOne, CancellationToken cancellationToken)
    {
    ...
    private static async Task NamedOptionsAsync(ISystemOneClient systemOne, CancellationToken cancellationToken)
    {
        ...
        ChoiceAnswer<string> answer = await systemOne.AskAsync(
            SampleData.ExampleTicket,
            department,
            cancellationToken);
    ...
    private static async Task ErrorsAsync(ISystemOneClient systemOne, CancellationToken cancellationToken)
    {
        ...
        {
            Console.WriteLine($"SystemOneValidationException: {exception.Message}");
        }
        ...
        try
        {
            await systemOne.NoulAsync(ticket, "Does this ticket convey urgency?", timeout.Token);
        ...
        Console.WriteLine();
        Console.WriteLine("catch (SystemOneApiException ex)      // non-success status; ex.StatusCode, ex.ResponseBody");
        Console.WriteLine("catch (SystemOneProtocolException ex)  // malformed or mismatched answer");
        Console.WriteLine("catch (SystemOneException ex)          // base class for both");
    }
}
